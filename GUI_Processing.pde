...
float cursor1X = 100; // x-coordinate of cursor 1
float cursor2X = 200; // x-coordinate of cursor 2
float cursorY = 20; // y-coordinate of both cursors
float cursorSize = 20; // size of both cursors
boolean dragging1 = false; // flag to indicate if cursor 1 is being dragged
...
void draw_cursors() {
  ...
  int boxY = 650;
  fill(255);
  rect(100, 650, boxWidth, boxHeight);
  fill(255);
  rect(boxX + 60, boxY, boxWidth, boxHeight);
  
  cursor1Time = map(cursor1X, 0, width, 0, nsamp * ADCPS[tbval]);
  cursor2Time = map(cursor2X, 0, width, 0, nsamp * ADCPS[tbval]);

  // Display peak-to-peak voltage inside the box
  fill(0, 0, 200);
  text("Cursor 1: " + nf(cursor1Time, 0, 2) + " ms", boxX + 80, boxY + 40);
  fill(0,200 , 0);
  text("Cursor 2: " + nf(cursor2Time, 0, 2) + " ms", boxX + 80, boxY + 20);
  
  // Display time for each cursor
  //fill(255);
  //text("Cursor 1: " + nf(cursor1Time, 0, 2) + " ms", 100, 650);
  //text("Cursor 2: " + nf(cursor2Time, 0, 2) + " ms", 100, 670);

  // draw cursors
  stroke(0, 0, 255);
  rect(cursor1X, cursorY, cursorSize/8, cursorSize);
  stroke(0,255 , 0);
  rect(cursor2X, cursorY, cursorSize/8, cursorSize);
  
  
  //print("Voltage difference between cursors: {:.3f} V".format(voltage_diff));
  
  // draw lines between cursors
  //stroke(0);
  //line(cursor1X+cursorSize/2, cursorY+cursorSize/2, cursor2X+cursorSize/2, cursorY+cursorSize/2);
}
...
for (int i = 0; i < nsamp; i++) {
  float voltage = (float) values[i] / maxval * (Vmax[ivb] / Vdiv[ivb]);
  if (voltage > peak) {
    peak = voltage;
  }
  if (voltage < trough) {
...
p2pVoltage = peak - trough;

String p2p = nf(p2pVoltage, 0, 2);

int boxWidth = 150;
  int boxHeight = 50;
  int boxX = width - boxWidth - 250;
  int boxY = 650;
  fill(255);
  ...
  // Display peak-to-peak voltage inside the box
  fill(0);
  text(" Peak2Peak: " + p2p + " V", boxX + 70, boxY + 30);
...
void mousePressed(){  //check for mouse clicks
  ...
  // check if cursor 1 is being clicked on
  if (mouseX >= cursor1X && mouseX <= cursor1X+cursorSize && mouseY >= cursorY && mouseY <= cursorY+cursorSize) {
    dragging1 = true;
  }
  // check if cursor 2 is being clicked on
  if (mouseX >= cursor2X && mouseX <= cursor2X+cursorSize && mouseY >= cursorY && mouseY <= cursorY+cursorSize) {
    dragging2 = true;
  }
...
void mouseReleased() {
  // release cursor 1 if it's being dragged
  if (dragging1) {
    dragging1 = false;
  }
  // release cursor 2 if it's being dragged
  if (dragging2) {
    dragging2 = false;
  }
}
...
void mouseDragged() {
  // move cursor 1 if it's being dragged
  if (dragging1) {
    cursor1X = mouseX - cursorSize/2;
    cursor1X = constrain(cursor1X, 0, cursor2X - cursorSize); // prevent cursor 1 from moving past cursor 2
  }
  // move cursor 2 if it's being dragged
  if (dragging2) {
    cursor2X = mouseX - cursorSize/2;
    cursor2X = constrain(cursor2X, cursor1X + cursorSize, width - cursorSize); // prevent cursor 2 from moving past cursor 1 or the right edge of the screen
  }
}
